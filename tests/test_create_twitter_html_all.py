#!/usr/bin/env python3
"""
create_twitter_html_all.pyのテスト
"""
...
class TestCreateTwitterHtmlAuto(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # pyautogui全体をモック
        self.pyautogui_patcher = patch('src.create_twitter_html_all.pyautogui', new=MagicMock())
    ...
    @patch('src.create_twitter_html_all.get_position')
    def test_get_position(self, mock_get_position):
        """位置取得機能のテスト（モック）"""
        mock_get_position.return_value = (100, 200)
        from src.create_twitter_html_all import get_position
    ...
    def test_save_html_to_file(self):
        """HTMLファイル保存機能のテスト"""
        from src.create_twitter_html_all import save_html_to_file
    ...
    def test_search_query_with_since(self):
        ...
        # テスト用の引数を設定
        test_args = ['create_twitter_html_all.py', '2025-01-15']
        ...
        with patch('sys.argv', test_args):
            with patch('pyperclip.paste') as mock_paste:
                ...
                with patch('src.create_twitter_html_all.get_position') as mock_get_position:
                    with patch('src.create_twitter_html_all.navigate_to_twitter_search') as clickmock_navigate:
                        with patch('src.create_twitter_html_all.copy_html_with_extension') as mock_copy:
                            with patch('src.create_twitter_html_all.save_html_to_file') as mock_save:
    ...
    def test_search_query_without_date(self):
        ...
        # テスト用の引数を設定
        test_args = ['create_twitter_html_all.py', '2025-01-15', '--no-date']
        ...
        with patch('sys.argv', test_args):
            with patch('pyperclip.paste') as mock_paste:
                ...
                # main関数をモックして、検索クエリ生成部分のみテスト
                with patch('src.create_twitter_html_all.get_position') as mock_get_position:
                    with patch('src.create_twitter_html_all.navigate_to_twitter_search') as mock_navigate:
                        with patch('src.create_twitter_html_all.copy_html_with_extension') as mock_copy:
                            with patch('src.create_twitter_html_all.save_html_to_file') as mock_save:
                                mock_get_position.return_value = (100, 100)
                                mock_copy.return_value = "<html>test</html>"
    ...
    def test_keyword_types(self):
        ...
        for keyword_type in keyword_types:
            with self.subTest(keyword_type=keyword_type):
                # テスト用の引数を設定
                test_args = ['create_twitter_html_all.py', '2025-01-15', '--keyword-type', keyword_type]
    ...
    def test_custom_keyword_input(self):
        ...
        # テスト用の引数を設定
        test_args = ['create_twitter_html_all.py', '2025-01-15', '--keyword-type', 'custom']
