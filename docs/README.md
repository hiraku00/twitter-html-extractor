...
タイムラインで「さらに表示」ボタンがあるツイートに対して、詳細ページから完全なテキストを自動取得する機能です。
...
```
さらに表示ボタン: あり
今日は良い天気ですね。朝から散歩に出かけました。公園でたくさんの花が咲いていて...
...
さらに表示ボタン: あり
完全なテキスト: あり
今日は良い天気ですね。朝から散歩に出かけました。公園でたくさんの花が咲いていて、とても綺麗でした。春の訪れを感じます。散歩の後はカフェでコーヒーを飲んでゆっくり過ごしました。午後は読書をする予定です。本当に良い休日になりました。
・天気が良いと心も晴れやかになりますね
・散歩は健康にも良いです
・花を見ているだけで癒されます
・カフェでのんびりする時間も大切です
...
### 処理の流れ
1. **タイムライン抽出**: HTMLからツイート要素を抽出し、「さらに表示」ボタンがあるかをチェックし、URLを記録
2. **詳細ページ処理**: 「さらに表示」ボタンがあるツイートに対して、毎回新しいタブを開き、URL欄を明示的に選択して詳細ページへ移動した後、ブラウザ拡張機能でHTMLをコピー
3. **テキスト統合**: コピーしたHTMLから完全なテキストを抽出し、省略されたテキストを完全なテキストで置き換え

詳細ページのHTMLは、対象ツイートの `status` ID と canonical URL が一致することを検証してから保存します。検索一覧ページがコピーされた場合やHTML取得に失敗した場合は、最大3回再試行し、対象外のHTMLを詳細ページとして保存しません。ブラウザ拡張機能のポップアップは操作の前後に閉じます。
...
# 複数キーワードを一括実行
python main.py all 260105 -k intmax,thai,chikirin
```
...
- `chikirin,thai` のようにカンマ区切りで指定することで、複数種類の検索を連続して実行
- 設定ファイルで定義されたキーワードタイプか、`--search-keyword` で直接キーワードを指定
- `chikirin` などのキーワードタイプを指定すると、専用フォルダにファイルが保存され、データが整理されます

`chikirin` の抽出に成功すると、抽出結果は自動的にdashboardのwatch-list APIへ登録されます。

- 登録先は環境変数 `WATCH_LIST_API_URL`（`.env`、`.env.example` 参照）
- 既存のX投稿URL（`sourceSystem: chikirin` の `externalId`）は重複登録しません
- `contentType` は `movie`、`sourceSystem` は `chikirin` として登録します
- 投稿本文をタイトル、X投稿リンクを参照リンクとして登録します
- dashboard APIへの登録に失敗しても、抽出済みデータの保存処理は継続します
...
| `python main.py all 260105 -k intmax,thai`         | 各キーワードで順次実行                                                           | 複数キーワードの一括実行                                         |
...
- **merge コマンド**
  ...
  - 使用可能なキーワードタイプ: `default`, `thai`, `en`, `chikirin`, `intmax`, `manekineko`, `custom`
...
- エラーハンドリングを強化
- 位置情報の保存/読み込み機能を追加
- 負のマウス座標（マルチディスプレイ環境）における座標永続化のバグを修正
- 複数キーワードのカンマ区切り指定に対応
- ログ出力を体系化し、デバッグを容易に
